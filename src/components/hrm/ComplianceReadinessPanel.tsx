  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
      ...
      {error ? <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div> : loading ? <p className="text-sm text-gray-500">Checking readiness…</p> : <>
        ...
        <div className="grid gap-2 md:grid-cols-2">{checks.map((c:any) => <div key={c.key} className="flex gap-2 rounded-lg border border-gray-200 dark:border-gray-700 p-3">{c.ok ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> : <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />}<div><p className="text-sm font-medium capitalize text-gray-900 dark:text-white">{String(c.key).replaceAll("_", " ")}</p><p className="text-xs text-gray-500 dark:text-gray-400">{c.detail}</p></div></div>)}</div>
      </>}
    </div>
  );
}
