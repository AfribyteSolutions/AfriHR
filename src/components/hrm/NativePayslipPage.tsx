...
export default function NativePayslipPage(){
 ...
 const download=(item:any)=>{const data={source:"AfriHR",employee:item.employee_name,employee_number:item.employee_number,currency:item.currency,period:item.snapshot,gross_minor:item.gross_minor,deductions_minor:item.deductions_total_minor,statutory_deductions_minor:item.statutory_deductions_total_minor,employer_contributions_minor:item.employer_contributions_total_minor,net_minor:item.net_minor,allowances:item.allowances||[],deductions:item.deductions||[],statutory_deductions:item.statutory_deductions||[],employer_contributions:item.employer_contributions||[]};const u=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)],{type:"application/json"})),a=document.createElement("a");a.href=u;a.download=`payslip-${item.snapshot?.period_end||item.id}.json`;a.click();URL.revokeObjectURL(u)};
 ...
 return <NativeHrShell title="My Payslips" subtitle="Finalized payroll statements"><main className="p-5 md:p-8 max-w-4xl mx-auto">
  {loading?<p className="py-16 text-center">Loading payslips…</p>:<div className="space-y-5">{items.map(i=><article key={i.id} className="bg-white dark:bg-slate-900 border rounded-2xl p-6 print:shadow-none">
   ...
   <div className="grid md:grid-cols-2 gap-5"><Lines title="Allowances" lines={i.allowances} currency={i.currency}/><Lines title="Other deductions" lines={i.deductions} currency={i.currency}/><Lines title="Statutory deductions" lines={i.statutory_deductions} currency={i.currency}/><Lines title="Employer contributions" lines={i.employer_contributions} currency={i.currency}/></div><p className="mt-5 text-xs text-slate-500">Statutory rules: {i.snapshot?.statutory_rule?.name||"—"} · version {i.snapshot?.statutory_rule?.version||"—"}</p>
