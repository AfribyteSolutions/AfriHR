...
export default function StatutoryRulesPage(){
 ...
 return <NativeHrShell title="Statutory Payroll Rules" subtitle="Effective-dated legal deductions and employer contributions"><main className="p-5 md:p-8 max-w-6xl mx-auto">
  ...
  <form onSubmit={save} className="bg-white dark:bg-slate-900 border rounded-2xl p-5 grid md:grid-cols-3 gap-4">
   ...
   <label className="text-sm font-bold mdEmployee deductions JSON<textarea value={form.employee_rules} onChange={e=>setForm({...form,employee_rules:e.target.value})} className="block w-full h-64 mt-1 p-3 font-mono text-xs border rounded-xl bg-transparent"/></label>
  ...
  <div className="space-y-3 mt-8">{rules.map(r=><article key={r.id} className="bg-white dark:bg-slate-900 border rounded-2xl p-5 flex flex-wrap justify-between gap-4"><div><h2 className="font-black">{r.name} · {r.version}</h2><p className="text-sm text-slate-500">{r.country_code}/{r.currency} · effective {r.effective_from} · verified {r.verified_on}</p><a href={r.source_url} target="_blank" rel="noreferrer" className="text-sm text-blue-600">Open official source</a></div></div></div><div className="text-right"><b className="capitalize">{r.status}</b>{r.status==="draft"&&<button disabled={busy} onClick={()=>void activate(r)} className="block mt-2 px-4 py-2 bg-emerald-600 text-white rounded-xl font-bold">Activate</button>}</div></article>)}{!loading&&!rules.length&&<p className="py-12 text-center border-2 border-dashed rounded-2xl text-slate-400">No statutory rule versions.</p>}</div>
 </main></NativeHrShell>
}
