...
export async function PATCH(
  ...
  try {
    ...
    const { totalProgress, ...otherUpdates } = body;

    // Validate totalProgress if provided
    if (
      totalProgress !== undefined &&
      (totalProgress < 0 || totalProgress > 100)
    ) {
      return NextResponse.json(
        { success: false, message: "Total progress must be between 0 and 100" },
        { status: 400 },
      );
    }
    ...
    if (totalProgress !== undefined) {
      updateData.totalProgress = totalProgress;
    }
