...
export default async function(req: Request) {
  ...
  const result = { tenant_id: tenantId, product: 'afrihr', readiness: blockers.length ? 'attention_required' : 'configured', legally_verified: false, configuration: { jurisdiction_code: tenant.jurisdiction_code || '', compliance_status: tenant.compliance_status || 'not_configured' }, ai_policy: dataOf((aiPolicies.data || aiPolicies)[0] || null), checks, blockers: blockers.map(b => b.key), note: 'Technical readiness only; not legal certification.' };
  if (tenant.external_org_key) {
    const snapshot = { action: 'publisher_ingest', external_org_key: tenant.external_org_key, organization_label: tenant.name || '', product_key: 'afrihr', source_app_id: '6aa2c086b488d732777073e1', source_record_id: tenantId, jurisdiction_code: tenant.jurisdiction_code || tenant.country || '', regional_contexts: [], readiness: result.readiness, compliance_status: tenant.compliance_status || 'not_configured', checks: checks.map(({key,label,ok,detail}:any) => ({key,label,ok,detail})), blockers: result.blockers, warnings: [], rule_versions: verifiedActive.map((r:any) => `${r.rule_key || r.name || r.id}:${r.version || 'verified'}`), integration_modes: ['africount_payroll_export'], source_updated_at: tenant.updated_date || null, notes: 'Technical readiness snapshot; no employee or payroll line-item data included.' };
    const recent = await S.GovernanceSnapshotOutbox.filter({ external_org_key: tenant.external_org_key, product_key: 'afrihr' }, '-created_date', 1).catch(() => []);
    const latest = dataOf((recent.data || recent)[0] || null);
    if (!latest || JSON.stringify(latest.payload) !== JSON.stringify(snapshot)) await S.GovernanceSnapshotOutbox.create({ external_org_key: tenant.external_org_key, product_key: 'afrihr', source_app_id: '6aa2c086b488d732777073e1', source_record_id: tenantId, payload: snapshot, status: 'pending', attempts: 0 });
  }
